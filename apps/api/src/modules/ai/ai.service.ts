...
export class AIService {
  ...
  async processNaturalLanguageQuery(
    ...
    switch (detectedIntent) {
      case 'projectStatus':
        return await this.handleProjectStatus(companyId, context?.projectId, entities);
      case 'budget':
        return await this.handleBudgetQuery(companyId, context?.projectId, context?.budgetId, entities);
      case 'schedule':
        return await this.handleScheduleQuery(companyId, context?.projectId, entities);
      case 'workers':
        return await this.handleWorkersQuery(companyId, context?.projectId);
      case 'recommendation':
        return await this.generateRecommendations(companyId, context?.projectId);
      case 'prediction':
  ...
  private extractEntities(query: string): any {
    ...
    const timePatterns = [
      { regex: /(\d+)\s*(día|días|semana|semanas|mes|meses)/i, key: 'timeframe' },
  ...
  private async handleProjectStatus(companyId: string, projectId?: string, _entities?: any): Promise<NLPQueryResult> {
    try {
      ...
      const summaries = projects.map((p) => {
        ...
        const executedPercentage = totalPrice > 0 ? (totalExecuted / totalPrice) * 100 : 0;
      ...
      const activeProjects = summaries.filter((s) => s.status === 'in_progress');
      ...
      return {
        answer,
        data: { projects: summaries, summary: { active: activeProjects.length, onTrack: onTrack.length } },
        sources: projects.map((p) => p.name),
        confidence: 0.85,
        actionable: true,
        suggestedActions: onTrack.length < summaries.length ? ['Revisar proyectos con desviación'] : ['Continuar monitoreando'],
      };
    } catch (error) {
  ...
  private async handleBudgetQuery(companyId: string, projectId?: string, budgetId?: string, _entities?: any): Promise<NLPQueryResult> {
    try {
      let budgets: Budget[];
      ...
      if (budgetId) {
        ...
        const projectIds = (await this.projectRepository.find({
          where: { company_id: companyId },
          select: ['id'],
          take: 5,
        })).map(p => p.id);
      ...
      if (budgets.length === 0) {
        return { answer: 'No encontré presupuestos.', confidence: 0.9, sources: [] };
      }
      ...
      const analysis = budgets.map((b) => {
        ...
        const variance = totalPrice > 0 ? ((totalExecuted - totalEstimated) / totalPrice) * 100 : 0;
        ...
        return {
          ...
          status: variance > 15 ? 'warning' : variance < -5 ? 'under' : 'on_track',
        };
      });
      ...
      const avgVariance = summary.totalPrice > 0 
        ? ((summary.totalExecuted - summary.totalEstimated) / summary.totalPrice) * 100 
        : 0;
      ...
      return {
        ...
        confidence: 0.9,
        actionable: true,
        suggestedActions: avgVariance > 10 ? ['Analizar causas de sobrecoste'] : avgVariance < 0 ? ['Revisar estimación'] : ['Mantener control'],
      };
    } catch (error) {
      console.error('Error in handleBudgetQuery:', error);
      return { answer: 'Error al consultar presupuestos.', confidence: 0.5, sources: [] };
    }
  }

  private async handleScheduleQuery(companyId: string, projectId?: string, _entities?: any): Promise<NLPQueryResult> {
    try {
      ...
      if (!projects.length) {
        return { answer: 'No hay proyectos activos.', confidence: 0.9, sources: [] };
      }
      ...
      const scheduleAnalysis = projects.map((p) => {
        ...
        const progress = Math.min(100, Math.max(0, totalDays > 0 ? (daysPassed / totalDays) * 100 : 0));
        
        let status: 'on_track' | 'warning' | 'delayed' = 'on_track';
        if (progress > 110) status = 'delayed';
        else if (progress > 100) status = 'warning';
        
        const expectedEnd = totalDays > 0 
          ? new Date(start.getTime() + (totalDays * 100 / Math.max(1, progress)) * 24 * 60 * 60 * 1000)
          : end;
        const delayDays = Math.floor((expectedEnd.getTime() - end.getTime()) / (1000 * 60 * 60 * 24));
      ...
      const delayed = scheduleAnalysis.filter((s) => s.status === 'delayed').length;
      const warning = scheduleAnalysis.filter((s) => s.status === 'warning').length;
      ...
      if (delayed === 0 && warning === 0) answer += ' Todos dentro del cronograma.';
      ...
      return {
        ...
        suggestedActions: delayed + warning > 0 ? ['Revisar cronogramas en riesgo'] : [],
      };
    } catch (error) {
      console.error('Error in handleScheduleQuery:', error);
      return { answer: 'Error al analizar cronograma.', confidence: 0.5, sources: [] };
    }
  }

  private async handleWorkersQuery(companyId: string, _projectId?: string): Promise<NLPQueryResult> {
    try {
      ...
      if (!workers.length) {
        return { answer: 'No hay trabajadores registrados.', confidence: 0.9, sources: [] };
      }

      const avgRating = workers.reduce((sum, w) => sum + (w.rating || 0), 0) / workers.length;
      const avgRate = workers.reduce((sum, w) => sum + (Number(w.daily_rate) || 0), 0) / workers.length;
      ...
      return {
        answer,
        data: { workers: workers.length, roles: roles.length, avgRate: Math.round(avgRate), avgRating: avgRating.toFixed(1) },
        sources: ['Workers'],
        confidence: 0.9,
      };
    } catch (error) {
      console.error('Error in handleWorkersQuery:', error);
      return { answer: 'Error al consultar trabajadores.', confidence: 0.5, sources: [] };
    }
  }

  async generateRecommendations(companyId: string, projectId?: string): Promise<NLPQueryResult> {
    const insights: AIInsight[] = [];

    try {
      const whereCondition: any = { company_id: companyId, status: 'in_progress' };
      if (projectId) whereCondition.id = projectId;
      ...
      for (const project of projects) {
        ...
        const progress = totalItems > 0 ? (completedItems / totalItems) * 100 : 0;
        const budgetProgress = totalCost > 0 ? (executedCost / totalCost) * 100 : 0;
        ...
        if (budgetProgress > progress + 20) {
          insights.push({
            ...
            metric: { label: 'Desviación', value: Math.round(budgetProgress - progress), change: budgetProgress - progress },
          });
        }
        ...
        if (progress > 80 && budget.status === 'editing') {
          insights.push({
            ...
            description: 'Proyecto avanzado, considera cerrar etapa de edición.',
            confidence: 0.9,
      ...
      return {
        answer: insights.map((i) => `• ${i.title}: ${i.description}`).join('\n'),
        data: { insights },
        sources: [],
        confidence: 0.8,
      ...
      return { answer: 'Error al generar recomendaciones.', confidence: 0.5, sources: [] };
    }
  }

  async predictProjectOutcome(companyId: string, projectId?: string): Promise<NLPQueryResult> {
    try {
      const whereCondition: any = { company_id: companyId, status: 'in_progress' };
      if (projectId) whereCondition.id = projectId;
      ...
      if (!projects.length) {
        return { answer: 'No hay proyectos para predecir.', confidence: 0.9, sources: [] };
      }
      ...
      const predictions: ProjectRiskPrediction[] = projects.map((p) => {
        ...
        const costVariance = totalCost > 0 ? (executedCost - totalCost) / totalCost : 0;
        const scheduleVariance = progress > 0 ? ((executedCost / Math.max(1, totalCost)) * 100) - progress : 0;

        const factors: { name: string; impact: number; weight: number }[] = [];
        if (Math.abs(costVariance) > 0.1) {
          factors.push({ name: 'Variación de costo', impact: Math.abs(costVariance) * 100, weight: 0.4 });
        }
        if (Math.abs(scheduleVariance) > 10) {
          factors.push({ name: 'Variación de cronograma', impact: Math.abs(scheduleVariance), weight: 0.3 });
        }
        if (itemCount > 0 && completedItems / itemCount < 0.5 && progress > 50) {
          factors.push({ name: 'Baja ejecución', impact: 20, weight: 0.3 });
        }

        const riskScore = factors.reduce((sum, f) => sum + f.impact * f.weight, 0);
        ...
        if (riskLevel !== 'low') {
          ...
          if (costVariance > 0) recommendations.push('Analizar causes de sobrecoste');
      ...
      return {
        ...
        suggestedActions: highRisk.length > 0 ? ['Revisar proyectos de riesgo'] : [],
      };
    } catch (error) {
      console.error('Error in predictProjectOutcome:', error);
      return { answer: 'Error al predecir resultados.', confidence: 0.5, sources: [] };
    }
  }

  private handleGeneralQuery(_companyId: string, _projectId: string | undefined, query: string): NLPQueryResult {
    const keywords = query.toLowerCase();
    
    if (keywords.includes('ayuda')) {
      return {
        answer: 'Puedo ayudarte con: estado de proyectos, presupuestos, cronograma, trabajadores, recomendaciones y predicciones. ¿Qué necesitas?',
        sources: [],
        confidence: 0.95,
      };
    }

    if (keywords.includes('hola') || keywords.includes('buenos') || keywords.includes('hello')) {
      return {
        answer: '¡Hola! Soy tu asistente de construcción. Puedo analizar tus proyectos, presupuestos, cronograma y dar recomendaciones. ¿En qué puedo ayudarte?',
        sources: [],
        confidence: 0.95,
      };
    }
    ...
    return {
      answer: 'No entendí tu pregunta. Puedo ayudarte con: estado de proyectos, presupuestos, cronograma, trabajadores, recomendaciones y predicciones.',
      sources: [],
      confidence: 0.6,
    };
  }
    ...
    try {
      ...
      for (const stage of budget.stages || []) {
        for (const item of stage.items || []) {
          ...
          const executed = Number(item.quantity_executed) * Number(item.unit_cost);
      ...
      analysis.variance = analysis.totalEstimated > 0 
        ? ((analysis.totalExecuted - analysis.totalEstimated) / analysis.totalEstimated) * 100 
        : 0;
  ...
  async generateProjectReport(projectId: string, type: 'executive' | 'financial' | 'technical'): Promise<any> {
    try {
      const project = await this.projectRepository.findOne({
        where: { id: projectId },
        relations: ['client', 'budgets', 'budgets.stages', 'budgets.stages.items'],
      });
      ...
      switch (type) {
        case 'executive':
          ...
          break;
        case 'financial':
          const financials = await this.financialService.getProjectSummary(projectId);
          report.sections.push({
