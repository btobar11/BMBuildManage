...
describe('BimApuLinkController', () => {
  ...
  describe('POST /', () => {
    it('should link element', async () => {
      const dto = { project_id: 'proj-1', item_id: 'item-1', ifc_global_id: 'ifc-1' };
      ...
      expect(mockService.linkElement).toHaveBeenCalledWith('company-1', dto, 'user-1');
    ...
    it('should handle link error', async () => {
      ...
      const user = { company_id: 'company-1', id: 'user-1' };
      await expect(controller.linkElement(user, { project_id: 'p1', item_id: 'i1', ifc_global_id: 'i1' } as any)).rejects.toThrow();
    });
  });
  ...
  describe('GET /project/:project_id', () => {
    it('should get links by project', async () => {
      ...
      expect(mockService.getLinksByProject).toHaveBeenCalledWith('company-1', 'proj-1');
      expect(result.success).toBe(true);
    ...
    it('should handle get project links error', async () => {
      ...
      const user = { company_id: 'company-1' };
      await expect(controller.getLinksByProject(user, 'proj-1')).rejects.toThrow();
    });
  });
  ...
  describe('GET /item/:item_id', () => {
    it('should get links by item', async () => {
      ...
      expect(mockService.getLinksByItem).toHaveBeenCalledWith('company-1', 'item-1');
      expect(result.success).toBe(true);
    });
  });
});